...
use crate::{Meter, Pid, Report, Snapshot, ThreadReport};
...
impl Meter {
    ...
    pub fn report(&self) -> Option<Report> {
        ...
        let mut cpu_usage = 100.0
            * (1.0 - (last.idle_time - prev.idle_time) as f32 / (centisecs * self.num_cpus as f32));
        if cpu_usage < 0. {
            // sometimes we get inaccuracy
            cpu_usage = 0.;
        }
        Some(Report {
            ...
            process_cpu_usage: 100.0
                * (lpro.user_time + lpro.system_time - (ppro.user_time + ppro.system_time)) as f32
                / centisecs,
            gross_cpu_usage: 100.0
                * ((lpro.user_time
                    + lpro.system_time
                    + lpro.child_user_time
                    + lpro.child_system_time)
                    - (ppro.user_time
                        + ppro.system_time
                        + ppro.child_user_time
                        + ppro.child_system_time)) as f32
                / centisecs,
            ...
            disk_read: (last.read_disk_bytes - prev.read_disk_bytes) as f32 / secs,
            disk_write: (last.write_disk_bytes - prev.write_disk_bytes) as f32 / secs,
            disk_cancelled: (last.write_cancelled_bytes - prev.write_cancelled_bytes) as f32 / secs,
